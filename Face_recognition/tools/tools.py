...
# Load the known image of sahil
known_image = face_recognition.load_image_file("sahil.jpg")
    ...
    for face_encoding in face_encodings:
        ...
        if distance < confidence_threshold:
            ...
            for face_location in face_locations:
                ...
                cv.putText(frame, 'sahil', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
            ...
            new_entry = pd.DataFrame({'Name': ['sahil'], 'Date': [date_str], 'Time': [time_str]})
            df = pd.concat([df, new_entry], ignore_index=True)

    if not recognized:
        cv.putText(frame, 'Not sahil', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                   (255, 0, 0), 2, cv.LINE_AA)

    # Check if recognized count has reached 5
    if recognition_count >= 5:
        # Save the DataFrame to an Excel file
        df.to_excel('attendence_save.xlsx', index=False)
